...
function decorate(){
 ...
 document.querySelectorAll<HTMLElement>('.vessel-dossier,.offer-detail').forEach(card=>{const kind=shipKind(card.querySelector('.label')?.textContent||card.textContent||'');addImage(card,'vessel-hero-art',SHIP_ART[kind],'append')});
